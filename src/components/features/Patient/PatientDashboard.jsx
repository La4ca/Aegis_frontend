...
const PatientDashboard = () => {
  ...
  const fetchPatientData = async () => {
    ...
    try {
      ...
      const patientRecord = patientsRes.data.find(
      ...
      if (patientRecord) {
        ...
        if (patientRecord.assignedDoctor) {
          try {
            const doctorRes = await api.get(
              "/users/" + patientRecord.assignedDoctor,
            );
        ...
        const logsRes = await api.get(
          "/health-logs?patientId=" + patientRecord._id,
        );
        setHealthLogs(logsRes.data);

        // Calculate comprehensive stats
        const heartRates = logsRes.data
          .map((log) => log.vitals?.heartRate)
          .filter((v) => v);
        const bpSystolic = logsRes.data
          .map((log) => log.vitals?.bloodPressure?.systolic)
          .filter((v) => v);
        const bpDiastolic = logsRes.data
          .map((log) => log.vitals?.bloodPressure?.diastolic)
          .filter((v) => v);
        const temperatures = logsRes.data
        ...
        setStats({
          ...
          averageBP: {
            systolic:
              bpSystolic.length > 0
                ? Math.round(
                    bpSystolic.reduce((a, b) => a + b, 0) / bpSystolic.length,
                  )
                : 0,
            diastolic:
              bpDiastolic.length > 0
                ? Math.round(
                    bpDiastolic.reduce((a, b) => a + b, 0) / bpDiastolic.length,
                  )
                : 0,
          },
          ...
          trends: {
            heartRate: getTrend(
              heartRates.slice(0, 5),
              heartRates.slice(5, 10),
            ),
            bloodPressure: getTrend(
              bpSystolic.slice(0, 5),
              bpSystolic.slice(5, 10),
            ),
            temperature: getTrend(
              temperatures.slice(0, 5),
              temperatures.slice(5, 10),
            ),
          },
        });
      }
  ...
  const downloadReport = () => {
    const reportData = {
      ...
      primaryDoctor: assignedDoctor
        ? "Dr. " +
          ...
          " " +
          (assignedDoctor.profile?.lastName || "")
        : "None assigned",
      healthSummary: stats,
      healthRecords: healthLogs.map((log) => ({
        ...
        recordedBy:
          "Dr. " +
          (log.recordedBy?.profile?.firstName || "") +
          " " +
    ...
    const exportFileDefaultName =
      "aegis_health_report_" + new Date().toISOString().split("T")[0] + ".json";
  ...
  const getTrendIcon = (trend) => {
    if (trend === "increasing")
      return <ArrowUp size={14} className="trend-up" />;
    if (trend === "decreasing")
      return <ArrowDown size={14} className="trend-down" />;
    return null;
  };
  ...
  const vitalCards = [
    ...
    },
    {
      ...
      normal: "120/80",
      status:
        latestVitals?.bloodPressure?.systolic <= 120 ? "normal" : "warning",
      color: "#3B82F6",
  ...
  const statusDistribution = [
    {
      name: "Critical",
      value: healthLogs.filter((l) => l.status === "critical").length,
      color: "#EF4444",
    },
    {
      name: "Warning",
      value: healthLogs.filter((l) => l.status === "warning").length,
      color: "#F59E0B",
    },
    {
      name: "Normal",
      value: healthLogs.filter((l) => l.status === "normal").length,
      color: "#10B981",
    },
  ];
        ...
        <div className="header-user">
          {patientData && (
            <Notifications
              ...
              onNotificationAction={fetchPatientData}
            />
          )}
      ...
      <main className="dashboard-main">
        ...
        <div className="dashboard-welcome">
          ...
          <div
            ...
            style={{
              background: getHealthStatusColor() + "20",
              color: getHealthStatusColor(),
            }}
          >
            <span
              className="status-dot"
              style={{ background: getHealthStatusColor() }}
            ></span>
            {stats.healthStatus}
          </div>
        </div>
        ...
        <div className="patient-stats-grid">
          ...
          <div className="patient-stat-card">
            ...
            <div className="stat-info">
              <h4>Avg BP</h4>
              <p className="stat-number">
                {stats.averageBP.systolic || "--"}/
                {stats.averageBP.diastolic || "--"} mmHg
              </p>
            </div>
          </div>
          <div className="patient-stat-card">
            ...
            <div className="doctor-info-content">
              ...
              <div>
                ...
                <div className="doctor-name">
                  Dr. {assignedDoctor.profile?.firstName}{" "}
                  {assignedDoctor.profile?.lastName}
                </div>
        ...
        {chartData.length > 0 ? (
          ...
          <div className="no-data">
            ...
            <p className="small">
              Your doctor will add health records during your consultations
            </p>
          </div>
        )}
        ...
        <div className="recent-logs">
          ...
          {loading ? (
            ...
            <div className="empty-state">
              ...
              <p className="small">
                When your doctor records your vitals, they will appear here
              </p>
            </div>
          ) : (
            <div className="logs-list">
              {healthLogs.slice(0, 10).map((log) => (
                <div
                  key={log._id}
                  className={"log-item log-status-" + (log.status || "normal")}
                >
                  <div className="log-header">
                    ...
                    <div className="log-doctor">
                      <Stethoscope size={12} />
                      Dr. {log.recordedBy?.profile?.firstName}{" "}
                      {log.recordedBy?.profile?.lastName}
                    </div>
                    <div
                      className={
                        "log-status status-" + (log.status || "normal")
                      }
                    >
                      {log.status === "critical"
                        ? "Critical"
                  ...
                  <div className="log-vitals">
                    ...
                    <div className="vital-detail">
                      <Activity size={14} />
                      <span>
                        BP: {log.vitals?.bloodPressure?.systolic || "--"}/
